...
inline auto select_support_mcl<t_b,t_pat_len>::select(size_type i)const -> size_type
{
    ...
    if (m_longsuperblock!=nullptr and !m_longsuperblock[sb_idx].empty()) {
        ...
        if ((offset&0x3F)==0) {
            ...
            size_type args = select_support_trait<t_b,t_pat_len>::args_in_the_first_word(*data, word_off, static_cast<uint8_t>(carry));

            if (args >= i) {
                return (word_pos<<6)+select_support_trait<t_b,t_pat_len>::ith_arg_pos_in_the_first_word(*data, i, word_off, static_cast<uint8_t>(carry));
            }
            word_pos+=1;
