...
struct rank_support_trait<0,1> {
    ...
    {
        return static_cast<uint32_t>(bits::cnt((~*(data+(idx>>6))) & bits::lo_set[idx&0x3F]));
    }

    static uint32_t full_word_rank(const uint64_t* data, size_type idx)
    {
        return static_cast<uint32_t>(bits::cnt((~*(data+(idx>>6)))));
    }
...
struct rank_support_trait<10,2> {
    ...
    static uint32_t word_rank(const uint64_t* data, size_type idx)
    {
        ...
        return static_cast<uint32_t>(bits::cnt(bits::map10(*data, carry) & bits::lo_set[idx&0x3F]));
    }
    ...
    static uint32_t full_word_rank(const uint64_t* data, size_type idx)
    {
        data = data+(idx>>6);
        uint64_t carry = (idx>63) ? *(data-1)>>63 : 0;
        return static_cast<uint32_t>(bits::cnt(bits::map10(*data, carry)));
    }
...
struct rank_support_trait<01,2> {
    ...
    static uint32_t word_rank(const uint64_t* data, size_type idx)
    {
        ...
        return static_cast<uint32_t>(bits::cnt(bits::map01(*data, carry) & bits::lo_set[idx&0x3F]));
    }
    ...
    static uint32_t full_word_rank(const uint64_t* data, size_type idx)
    {
        data = data+(idx>>6);
        uint64_t carry = (idx>63) ? *(data-1)>>63 : 1;
        return static_cast<uint32_t>(bits::cnt(bits::map01(*data, carry)));
    }
...
struct rank_support_trait<00,2> {
    ...
    static uint32_t word_rank(const uint64_t* data, size_type idx)
    {
        data = data+(idx>>6);
        uint64_t carry = (idx>63) ? *(data-1)>>63 : 1;
        return static_cast<uint32_t>(bits::cnt((~(*data | ((*data)<<1 | carry))) & bits::lo_set[idx&0x3F]));
    }
    ...
    static uint32_t full_word_rank(const uint64_t* data, size_type idx)
    {
        data = data+(idx>>6);
        uint64_t carry = (idx>63) ? *(data-1)>>63 : 1;
        return static_cast<uint32_t>(bits::cnt(~(*data | ((*data)<<1 | carry))));
    }
