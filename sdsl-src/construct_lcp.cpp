...
void construct_lcp_go(cache_config& config)
{
    ...
    // phase 1: calculate lcp_sml;  memory consumption: 2n bytes (lcp_sml=n bytes, text=n bytes)
    {
        ...
        alphabet[sigma] = '\0';
        {
            ...
            uint8_t bwti_1 = static_cast<uint8_t>(bwt_buf[0]);       // store value of BWT[i-1]
            ...
            for (size_type i=1, sai, cur_c_idx=1, cur_c_cnt=cnt_c[alphabet[1]+1]; i < n; ++i, --cur_c_cnt) {
                uint8_t bwti = static_cast<uint8_t>(bwt_buf[i]);
                sai = sa_buf[i];
                ...
                if (i >= cnt_cc[cur_c]) { // if the current lcp entry is not already done   TODO: schleife von i bis cnt_cc[cur_c]
                    if (bwti == bwti_1 and lf < i) {  // BWT[i]==BWT[i-1]
                        ...
                        lcp_sml[i] = static_cast<uint8_t>(l);
                        ++done_cnt;
                ...
                if (lf > i) {   // if LF[i] > i, we can calculate LCP[LF[i]] in constant time with rmq
                    ...
                    lcp_sml[lf] = static_cast<uint8_t>(rmq_stack[j+3] - (rmq_stack[j+3]==m+2)); // if lcp-value equals m+1, we subtract 1
                }
                if (l >= m) {
                    if (l == m)
...
//			std::cout<<"# begin calculating LF' values"<<std::endl;
        int_vector<> lcp_big(nn, 0, bits::hi(n-1)+1); // lcp_big first contains adapted LF values and finally the big LCP values
        {
            ...
            for (size_type i=0, i2=0; i < n; ++i) {
                uint8_t b = static_cast<uint8_t>(bwt_buf[i]);  // store BWT[i]
...
//			std::cout<<"# begin initializing bwt2, shift_bwt2, run2"<<std::endl;
        ...
        run2[nn] = 0;							// index nn is not a big LCP value
        {
            ...
            bool is_run = false;
            for (size_type i=0, i2=0; i < n; ++i) {
                uint8_t b = static_cast<uint8_t>(bwt_buf[i]);
                if (lcp_sml_buf[i] >= m) {
                    bwt2[i2] 		= b;
                    shift_bwt2[i2]	= b_1;
                    run2[i2] 		= is_run;
...
void construct_lcp_bwt_based(cache_config& config)
{
    ...
    size_type bb = static_cast<size_type>((n*20-size_in_bytes(wt_bwt)*8*1.25-5*n)/n); 	// 20n - size of wavelet tree * 1.25 for rank support - 5n for bit arrays - n for index_done array
    if (n*20 < size_in_bytes(wt_bwt)*8*1.25+5*n) {
        bb = 6;
    }
    bb = std::min(bb, static_cast<size_type>(8));
    ...
    // init partial_lcp
    int_vector<> partial_lcp(n, 0, static_cast<uint8_t>(bb));   // LCP array
    ...
    while (intervals) {
        ...
        if (lcp_value>=lcp_value_max) {
            ...
            uint8_t int_width_new = static_cast<uint8_t>(std::min(space_in_bit_for_lcp / remaining_lcp_values , (size_type)bits::hi(n-1)+1));
